...
class RealTreeTest(unittest.TestCase):
    ...
    def test_model_matches_the_research_counts(self):
        ...
        jaxb_enums = [e for e in self.ir['enums'] if not e['core']]
        self.assertEqual(len(jaxb_enums), 115, '87 annotated enums plus the unannotated enums of bound properties')
        self.assertEqual(len(self.ir['enums']) - len(jaxb_enums), 141, 'S0a: the other 142 enums of game-server/src minus [hand_written_enums]')
        self.assertEqual(self.cm.model.hand_written_enums, ['com.aionemu.gameserver.configs.main.SecurityConfig.MultiClientingRestrictionMode'])
    ...
    def test_every_behaviour_class_has_its_hand_written_files(self):
        """S0a decision 4: `xmlgen.py scaffold --all` wrote the shells once; they are hand-owned since then, so only their existence is
        checked (a new behaviour class after a generator or Java change needs `scaffold --all` again)."""
        import scaffold
        src = os.path.join(REPO, 'cpp', 'game-server', 'src')
        missing = [rel for rel, _ in scaffold.scaffold_all(self.cm) if not os.path.isfile(os.path.join(src, rel))]
        self.assertEqual(missing, [], 'run python cpp/tools/xmlgen/xmlgen.py scaffold --all')

    def test_skeleton_drafts_do_not_redefine_generated_enums(self):
        """skeleton.py --draft of outer classes of generated nested enums (aliases) and of a file with a secondary generated enum passes
        the conflict check of `xmlgen.py generate`/`check`; a nested definition in the draft would not."""
        import tempfile
        from gen import skeleton
        project = skeleton.Project(skeleton.DEFAULT_JAVA_ROOT, None, skeleton.DEFAULT_CPP_SRC, skeleton.DEFAULT_COMMONS_SRC, None,
                                   skeleton.DEFAULT_GENERATED_ROOT)
        p = 'com.aionemu.gameserver.'
        drafts = skeleton.generate_drafts(project, project.select([p + 'network.aion.AionConnection', p + 'services.RecallService',
                                                                   p + 'services.abyss.AbyssSkillService']))
        connection = drafts['aion/gameserver/network/aion/AionConnection.h']
        self.assertIn('\tusing State = ::aion::gameserver::network::aion::AionConnection_State; // generated by xmlgen\n', connection)
        self.assertIn('#include "aion/gameserver/network/aion/AionConnection_State.h"', connection)
        with tempfile.TemporaryDirectory() as src:
            for rel, text in drafts.items():
                xmlgen.write_text(os.path.join(src, rel), text)
            xmlgen.check_src_conflicts(self.files, src, self.cm)
            xmlgen.write_text(os.path.join(src, 'aion/gameserver/network/aion/AionConnection.h'),
                              connection.replace('\tusing State = ', '\tenum class State : std::uint8_t { CONNECTED };\n\tusing X = '))
            with self.assertRaisesRegex(jaxb.XmlGenError, r'network.aion.AionConnection.State'):
                xmlgen.check_src_conflicts(self.files, src, self.cm)

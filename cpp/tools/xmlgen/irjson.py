...
- staticdata-classes.json (format aion-staticdata-classes, version 1): the reachable K1 classes and JAXB enums with their runtime-mutable
  fields, for tools/gen/fieldmap.py (runtime-architecture.md §3.1: K1 classes are exempt from K4 rules and escape analysis). The core enums
  of the policy flag core_enums are not static data and appear in xmlmodel.json only (`core: true`).
...
def staticdata_classes(cm):
    ...
    for e in sorted(cm.enums.values(), key=lambda e: e.fqn):
        if e.model.core:
            continue  # not static data: the core enums are listed in xmlmodel.json only
...
def report(cm, files):
    ...
    lines = ['# xmlgen report', '', 'Generated by `python cpp/tools/xmlgen/xmlgen.py generate` from the Java sources (docs/design/static-data.md §1.5).', '',
             ...
             f'| Behaviour classes (hand-written, generated member blocks) | {len(behaviour)} |',
             f'| Enums bound by JAXB | {sum(1 for e in cm.enums.values() if not e.model.core)} |',
             f'| Core enums (not bound by JAXB, policy core_enums) | {sum(1 for e in cm.enums.values() if e.model.core)} |',
    ...
    lines += ['', '## Enums with constructor data or methods (hand-written companion headers)', '',
              'Constant-specific bodies cannot be emitted mechanically either: those enums get their constants and name tables, the bodies are',
              'hand-written with the companion functions.', '']
    for e in sorted(cm.enums.values(), key=lambda e: e.fqn):
        if e.model.has_methods or e.model.has_constructor_args or e.model.has_fields or e.model.has_constant_bodies:
            flags = [x for x, f in (('methods', e.model.has_methods), ('constructor arguments', e.model.has_constructor_args),
                                    ('fields', e.model.has_fields), ('constant bodies', e.model.has_constant_bodies),
                                    ('core', e.model.core)) if f]
            lines.append(f'- {short_fqn(e.fqn)} ({", ".join(flags)})')
    lines += ['', '## Nested enums without a generated alias (the hand-written outer type declares `using Inner = Outer_Inner;`)', '']
    lines += [f'- {short_fqn(e.fqn)} -> `{e.qualified}`' for e in sorted(cm.enums.values(), key=lambda e: e.fqn)
              if e.outer_alias is not None and not any(cc.fqn == e.outer_alias[0] for cc in cm.classes.values())] or ['- none']
    lines += ['', '## Core enums with a hand-written C++ definition (xmlgen.toml [hand_written_enums], not generated)', '']
    lines += [f'- {short_fqn(x)}: {cm.policy.hand_written_enums[x]["reason"]}' for x in m.hand_written_enums] or ['- none']

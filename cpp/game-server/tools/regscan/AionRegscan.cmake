...
# entry type and accessor of each table (HandlerRegistry.h; the tool's Emitter.cpp uses the same names)
set(AION_GS_REGISTRY_ai_ENTRY AIHandlerEntry)
set(AION_GS_REGISTRY_ai_ACCESSOR aiHandlerEntries)
set(AION_GS_REGISTRY_instance_ENTRY InstanceHandlerEntry)
set(AION_GS_REGISTRY_instance_ACCESSOR instanceHandlerEntries)
set(AION_GS_REGISTRY_zone_ENTRY ZoneHandlerEntry)
set(AION_GS_REGISTRY_zone_ACCESSOR zoneHandlerEntries)
set(AION_GS_REGISTRY_quest_ENTRY QuestHandlerEntry)
set(AION_GS_REGISTRY_quest_ACCESSOR questHandlerEntries)
set(AION_GS_REGISTRY_commands_ENTRY CommandEntry)
set(AION_GS_REGISTRY_commands_ACCESSOR commandEntries)
set(AION_GS_REGISTRY_clientpackets_ENTRY ClientPacketEntry)
set(AION_GS_REGISTRY_clientpackets_ACCESSOR clientPacketEntries)
set(AION_GS_REGISTRY_npcids_ENTRY int32_t)
set(AION_GS_REGISTRY_npcids_ACCESSOR npcIdsSpawnedByHandlers)
...
function(aion_gs_add_registries name)
	...
	# The empty tables do not depend on any scanned file: the <name>_<r>_empty libraries compile copies written here at configure time (the same
	# text as the tool's .empty.gen.cpp, which the tool still writes next to the real tables), so unit tests that link them neither wait for the
	# scan nor fail when a handler file breaks a scan rule.
	set(empty_dir "${ARG_OUT_DIR}/empty")
	foreach(registry IN LISTS AION_GS_REGISTRY_NAMES)
		set(entry_type "${AION_GS_REGISTRY_${registry}_ENTRY}")
		set(accessor "${AION_GS_REGISTRY_${registry}_ACCESSOR}")
		file(CONFIGURE OUTPUT "${empty_dir}/Registry.${registry}.empty.gen.cpp" NEWLINE_STYLE LF CONTENT "\
// Registry.${registry}.empty.gen.cpp - generated by aion_gs_regscan from nothing (the empty variant for tests without handlers). Do not edit.
#include \"aion/gameserver/handlers/HandlerRegistry.h\"

namespace aion::gameserver::handlers {

std::span<const ${entry_type}> ${accessor}() noexcept {
	return {};
}
...
")
	...
	foreach(registry IN LISTS AION_GS_REGISTRY_NAMES)
		...
		add_library(${name}_${registry}_empty STATIC "${empty_dir}/Registry.${registry}.empty.gen.cpp")
		add_dependencies(${name}_${registry} ${name}_scan)

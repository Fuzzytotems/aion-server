...
// Shared fixture of the P5-09 economy tests (m5a-plan.md E2-01..E2-03).
//
// Database: a fresh database `aion_gs_test_economy` on the server named by AION_TEST_GS_DATABASE_URL (the database of the URL is only used for the
// lock connection), created from game-server/sql/aion_gs.sql of the Java tree, like the DAO tests (tests/dao/DaoTestDatabase.h, whose rules this
// copy follows). ctest runs every test case in its own process, so the singletons a test constructs (BrokerService, AtreianPassportService, ...)
...
inline constexpr std::string_view TEST_DATABASE = "aion_gs_test_economy";
...
#define ECONOMY_REQUIRE_DATABASE()                                                                                                                    \
	if (!requireDatabase())                                                                                                                           \

...
if(TARGET aion_gs_scenario_tests)
	...
	# interpreter CMake found handed over the way the gates below get theirs; without one the case skips itself. The properties script is
	# written here and not through aion_set_discovered_test_properties, because that helper passes PROPERTIES through a CMake list and so splits
	# a list value: LABELS "scenario;realdata" arrives as LABELS scenario, and the words after it pair up wrongly (`ctest -N
	# --show-only=json-v1` shows LoginServerHarnessTest with LABELS [scenario] and neither its RESOURCE_LOCK nor its TIMEOUT).
	set(oracle_run_environment "")
	if(Python3_Interpreter_FOUND)
		set(oracle_run_environment " ENVIRONMENT_MODIFICATION [==[AION_TEST_PYTHON=set:${Python3_EXECUTABLE}]==]")
	endif()
	set(oracle_run_script "${CMAKE_CURRENT_BINARY_DIR}/aion_gs_scenario_tests_oracle_run_properties.cmake")
	file(CONFIGURE OUTPUT "${oracle_run_script}" CONTENT "# generated by tests/scenario/ScenarioTests.cmake
foreach(aion_test IN LISTS aion_gs_scenario_tests_TESTS)
	if(aion_test MATCHES [==[^OracleRunTest\\.]==])
		set_tests_properties(\"\${aion_test}\" PROPERTIES LABELS [==[scenario;realdata]==]@oracle_run_environment@)
	endif()
endforeach()
" @ONLY)
	set_property(DIRECTORY APPEND PROPERTY TEST_INCLUDE_FILES "${oracle_run_script}")

...
int32_t BonusPackDAO::loadReceivingPlayer(int32_t accountId) {
	try {
		...
		log.error("[BONUS_PACK] Error loading received player id on account id " + std::to_string(accountId), e);
...
bool BonusPackDAO::storeReceivingPlayer(int32_t accountId, int32_t playerId) {
	try {
		...
		log.error("[BONUS_PACK] Error saving received player id " + std::to_string(playerId) + " on account id " + std::to_string(accountId), e);
		return false;
	}
}

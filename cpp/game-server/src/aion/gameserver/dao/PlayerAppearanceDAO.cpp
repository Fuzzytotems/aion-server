...
runtime::Ref<model::gameobjects::player::PlayerAppearance> PlayerAppearanceDAO::load(int32_t playerId) {
	runtime::Ref<PlayerAppearance> pa = PlayerAppearance::create();
	try {
		auto con = DatabaseFactory::getConnection();
		auto stmt = con->prepareStatement("SELECT * FROM player_appearance WHERE player_id = ?");
		...
		log.error("Could not restore PlayerAppearance data for player " + std::to_string(playerId) + " from DB: " + e.what(), e);
		return nullptr;
	}
	return pa;
}
...
bool PlayerAppearanceDAO::store(int32_t id, model::gameobjects::player::PlayerAppearance& pa) {
	// the adjacent literals are the parts of the Java string concatenation
	return DB::insertUpdate("REPLACE INTO player_appearance ("
							...
							"leg_thickness, leg_length, foot_size, facial_rate, voice, height)"
							" VALUES "
							"(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,"
							"?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?, ?"
							")",

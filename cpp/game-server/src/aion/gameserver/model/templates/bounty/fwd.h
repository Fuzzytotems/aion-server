// GENERATED by cpp/tools/gen/skeleton.py --fwd from the Java package com.aionemu.gameserver.model.templates.bounty - do not edit.
// Regenerate after adding a Java class or regenerating xmlgen: python cpp/tools/gen/skeleton.py --fwd --out cpp/game-server/src
#pragma once

#include <cstdint>

namespace aion::gameserver::model::templates::bounty {

struct BountyTemplate;
enum class BountyType : std::uint8_t;
struct KillBountyTemplate;

} // namespace aion::gameserver::model::templates::bounty

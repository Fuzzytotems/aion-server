// GENERATED by cpp/tools/gen/skeleton.py --fwd from the Java package com.aionemu.gameserver.skillengine.periodicaction - do not edit.
// Regenerate after adding a Java class or regenerating xmlgen: python cpp/tools/gen/skeleton.py --fwd --out cpp/game-server/src
#pragma once

namespace aion::gameserver::skillengine::periodicaction {

class HpUsePeriodicAction;
class MpUsePeriodicAction;
class PeriodicAction;
struct PeriodicActions;

} // namespace aion::gameserver::skillengine::periodicaction

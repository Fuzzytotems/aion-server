...
bool ChargeArmorCondition::validate(model::Skill& env) const {
	if (Ptr<Player> effector = runtime::as<Player>(env.getEffector())) {
		for (Ptr<Item> item : effector->getEquipment().getEquippedItems()) {
			if (item->getItemTemplate()->isArmor() && item->getConditioningInfo()) {
				if ((item->getEquipmentSlot() & gameserver::model::items::getSlotIdMask(ItemSlot::SUB_OFF_HAND)) != 0)
					continue;

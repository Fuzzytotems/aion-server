// GENERATED by cpp/tools/gen/skeleton.py --fwd from the Java package com.aionemu.gameserver.geoEngine.scene - do not edit.
// Regenerate after adding a Java class or regenerating xmlgen: python cpp/tools/gen/skeleton.py --fwd --out cpp/game-server/src
#pragma once

namespace aion::gameserver::geoEngine::scene {

class CollisionData;
class DespawnableNode;
class Geometry;
class Mesh;
class Node;
class Spatial;

// Nested types (declared inside their outer class, include its header):
//   DespawnableNode::DespawnableType
//   Spatial::CullHint

} // namespace aion::gameserver::geoEngine::scene

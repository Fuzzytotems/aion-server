...
IndexByteArray::IndexByteArray(runtime::Ref<runtime::Array<int8_t>> bufValue) : buf(std::move(bufValue)) {
}

IndexByteArray::~IndexByteArray() = default;

runtime::Ref<IndexByteArray> IndexByteArray::create(runtime::Ref<runtime::Array<int8_t>> bufValue) {
	return runtime::makeRef<IndexByteArray>(std::move(bufValue));
}
...
int32_t IndexByteArray::get(int32_t i) {
	return buf->get(i) & 0xFF;
}

int32_t IndexByteArray::size() {
	return buf->length();
}

void IndexByteArray::swap(int32_t i1, int32_t i2) {
	int32_t p1 = i1 * 3;
	int32_t p2 = i2 * 3;
	utils::TempVars& vars = utils::TempVars::get();
	utils::TempVars::ReleaseGuard releaseOnException(vars);
	runtime::Array<int8_t>& array = *buf;
	// Java System.arraycopy checks both ranges before copying
	if (p1 < 0 || p2 < 0 || p1 > array.length() - 3 || p2 > array.length() - 3)
		throw runtime::ArrayIndexOutOfBoundsException("arraycopy: last source index " + std::to_string(std::max(p1, p2) + 3) + " out of bounds for byte[" +
			std::to_string(array.length()) + "]");
	// store p1 in tmp
	for (int32_t k = 0; k < 3; ++k)
		vars.bihSwapTmp[static_cast<size_t>(k)] = array.get(p1 + k);
	...
	// copy tmp to p2
	for (int32_t k = 0; k < 3; ++k)
		array[p2 + k].set(vars.bihSwapTmp[static_cast<size_t>(k)]);
	vars.release();
}

int32_t IndexByteArray::hashCode() const {
	// Java record hashCode: the identity hash of the array (not reproducible; any value consistent with identity equality)
	return static_cast<int32_t>(std::hash<const runtime::Array<int8_t>*>()(buf.get()));
}

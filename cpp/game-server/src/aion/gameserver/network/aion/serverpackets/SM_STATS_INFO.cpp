...
SM_STATS_INFO::SM_STATS_INFO(model::gameobjects::player::Player& playerValue)
	...
	pcd = runtime::Ref<model::gameobjects::player::PlayerCommonData>(playerValue.getCommonData());
	pgs = runtime::Ref<model::stats::container::PlayerGameStats>(playerValue.getGameStats());
	pls = runtime::Ref<model::stats::container::PlayerLifeStats>(playerValue.getLifeStats());
}
...
void SM_STATS_INFO::writeImpl(AionConnection* con) {
	...
	writeH(pgs->getStat(StatEnum::WATER_RESISTANCE, 0.0f)->getCurrent()); // [current water]
	writeH(pgs->getStat(StatEnum::WIND_RESISTANCE, 0.0f)->getCurrent()); // [current wind]
	writeH(pgs->getStat(StatEnum::EARTH_RESISTANCE, 0.0f)->getCurrent()); // [current earth]
	writeH(pgs->getStat(StatEnum::FIRE_RESISTANCE, 0.0f)->getCurrent()); // [current fire]
	writeH(pgs->getStat(StatEnum::ELEMENTAL_RESISTANCE_LIGHT, 0.0f)->getCurrent()); // [current light resistance]
	writeH(pgs->getStat(StatEnum::ELEMENTAL_RESISTANCE_DARK, 0.0f)->getCurrent()); // [current dark resistance]
	...
	writeH(pgs->getMainHandPAttack(DISPLAY)->getCurrent()); // [current main hand attack]
	writeH(pgs->getOffHandPAttack(DISPLAY)->getCurrent()); // [current off hand attack]
	writeH(0); // unk 3.0
	writeD(pgs->getPDef()->getCurrent()); // [current pdef]
	writeH(pgs->getMainHandMAttack(DISPLAY)->getCurrent()); // [current magic attack]
	writeH(pgs->getOffHandMAttack(DISPLAY)->getCurrent()); // [current off hand magic attack]
	writeD(pgs->getMDef()->getCurrent()); // [Current magic def]
	writeH(pgs->getMResist()->getCurrent()); // [current mres]
	writeH(0); // unk 3.0
	writeF(pgs->getAttackRange()->getCurrent() / 1000.0f); // attack range
	...
	writeH(0); // [unk]
	writeF(pgs->getReverseStat(StatEnum::BOOST_CASTING_TIME, 1000.0f)->getCurrent() / 1000.0f); // [current casting speed]
	writeH(0); // [unk 3.5]
	writeH(pgs->getStat(StatEnum::CONCENTRATION, 0.0f)->getCurrent()); // [current concetration]
	writeH(pgs->getMBoost()->getCurrent()); // [current magic boost]
	writeH(pgs->getMBResist()->getCurrent()); // [current magic suppression]
	writeH(pgs->getStat(StatEnum::HEAL_BOOST, 0.0f)->getCurrent()); // [current heal_boost]
	writeH(0); // unk
	writeH(pgs->getPCR()->getCurrent()); // [current strike resist]
	writeH(pgs->getMCR()->getCurrent()); // [current spell resist]
	writeH(pgs->getStat(StatEnum::PHYSICAL_CRITICAL_DAMAGE_REDUCE, 0.0f)->getCurrent()); // [current strike fortitude]
	writeH(pgs->getStat(StatEnum::MAGICAL_CRITICAL_DAMAGE_REDUCE, 0.0f)->getCurrent()); // [current spell fortitude]
	writeD(player->getInventory().getLimit());
	writeD(player->getInventory().size());
	writeD(0); // [unk]
	writeD(0); // [unk]
	writeD(model::getClassId(pcd->getPlayerClass())); // [Player Class id]
	writeH(0); // unk 3.0
	writeH(0); // unk 3.0
	writeH(0); // [unk 3.5]
	writeH(0); // [unk 3.5]
	...
	writeH(pgs->getStat(StatEnum::WATER_RESISTANCE, 0.0f)->getBase()); // [base water res]
	writeH(pgs->getStat(StatEnum::WIND_RESISTANCE, 0.0f)->getBase()); // [base water res]
	writeH(pgs->getStat(StatEnum::EARTH_RESISTANCE, 0.0f)->getBase()); // [base earth resist]
	writeH(pgs->getStat(StatEnum::FIRE_RESISTANCE, 0.0f)->getBase()); // [base water res]
	writeH(pgs->getStat(StatEnum::ELEMENTAL_RESISTANCE_LIGHT, 0.0f)->getBase()); // [base light resistance]
	writeH(pgs->getStat(StatEnum::ELEMENTAL_RESISTANCE_DARK, 0.0f)->getBase()); // [base dark resistance]
	...
	writeH(pgs->getMainHandPAttack(DISPLAY)->getBase()); // [base main hand attack]
	writeH(pgs->getOffHandPAttack(DISPLAY)->getBase()); // [base off hand attack]
	writeH(pgs->getMainHandMAttack(DISPLAY)->getBase()); // [base main hand magic attack]
	writeH(pgs->getOffHandMAttack(DISPLAY)->getBase()); // [base off hand magic attack]
	...
	writeH(pgs->getStat(StatEnum::CONCENTRATION, 0.0f)->getBase()); // [base concentration]
	writeH(pgs->getMBoost()->getBase()); // [base magic boost]
	writeH(pgs->getMBResist()->getBase()); // [base magic suppression]
	writeH(pgs->getStat(StatEnum::HEAL_BOOST, 0.0f)->getBase()); // [base healboost]
	writeH(0); // unk
	writeH(pgs->getPCR()->getBase()); // [base strike resist]
	writeH(pgs->getMCR()->getBase()); // [base spell resist]
	writeH(pgs->getStat(StatEnum::PHYSICAL_CRITICAL_DAMAGE_REDUCE, 0.0f)->getBase()); // [base strike fortitude]
	writeH(pgs->getStat(StatEnum::MAGICAL_CRITICAL_DAMAGE_REDUCE, 0.0f)->getBase()); // [base spell fortitude]
}

...
SM_GM_SHOW_PLAYER_STATUS::SM_GM_SHOW_PLAYER_STATUS(model::gameobjects::player::Player& playerValue)
	: AionServerPacket(opcodeOf<SM_GM_SHOW_PLAYER_STATUS>), player(playerValue), pgs(playerValue.getGameStats()), pls(playerValue.getLifeStats()),
	  pcd(playerValue.getCommonData()) {
}
...
void SM_GM_SHOW_PLAYER_STATUS::writeImpl(AionConnection* con) {
	...
	writeH(0); // [unk]
	writeF(pgs->getReverseStat(StatEnum::BOOST_CASTING_TIME, 1000)->getCurrent() / 1000.0f); // [current casting speed]
	writeH(0); // [unk 3.5]

...
/**
 * This class contains possible response that LoginServer may send to client. The enumerator values are the ids sent to the client.
 * <p>
 * Java: com.aionemu.loginserver.network.aion.AionAuthResponse
 *
 * @author KID, Neon
 */
enum class AionAuthResponse : int32_t {
	/** Everything is OK. */
	...
	/**
	 * The telephone authentication service is unavailable as there is no phone connection or the line is busy. Please try again after moving to
	 * where there is phone connection or hanging up the phone.
	 */
...
/** Java: AionAuthResponse.getId() */
constexpr int32_t getId(AionAuthResponse response) noexcept {
	return static_cast<int32_t>(response);
}

/** Java: AionAuthResponse.getByIdOrDefault(id, defaultValue) */
constexpr AionAuthResponse getByIdOrDefault(int32_t id, AionAuthResponse defaultValue) noexcept {
	return id >= getId(AionAuthResponse::STR_L2AUTH_S_ALL_OK) && id <= getId(AionAuthResponse::STR_L2AUTH_S_ACCOUNTCACHESERVER_DOWN)
		? static_cast<AionAuthResponse>(id)
		: defaultValue;
}

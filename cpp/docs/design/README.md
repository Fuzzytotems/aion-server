...
| [phase4-status.md](phase4-status.md) | Status of phase 4 (bodies behind the frozen spine, towards M4): per-chunk table with measured `AION_UNPORTED` counts, tests and cross-chunk needs; wave 3a-1 results (ported classes, review and fix outcomes, header request decisions, deviations and Java races per chunk, deferred items); wave 3a-2 |

...
| Event buffs | `SkillEngine::applyEffectDirectly(skillId, …, duration, forceType)` (ported) → `XPBoostEffect::calculate` (`XPBoostEffect.cpp`, P5-04; **ported by M5b-3 stage 1**, m5b3-plan.md §16 — was unported when this row was written) for 10821/10825; `DRBoostEffect`, `SkillXPBoostEffect`, `APBoostEffect` are data-only on the ported `BufEffect`; `StatupEffect` (M5b-2 subset) for 18141/21258, which are applied only inside group/alliance instances (§2.4) | M5i (1 body, lease) |

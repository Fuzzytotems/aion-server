...
7. Zero `AION_UNPORTED` hits on this path. Load time and peak working set are logged and recorded in [phase4-status.md](phase4-status.md) (Debug gate build and RelWithDebInfo, before and after the wave 3b-2 performance work); they are informational, not pass/fail.
...
| P5-02a / P5-02b | `aion_gs_skills` (two parts) | the cast state machine (SkillEngine, Skill, conditions, actions, changes, periodic actions, tasks) / what a cast produces (Effect, effect modifiers, controllers.effect, model.skill) | 9,475 (114) | 115 | P5-01 (paired, close coordination) | 5a; split for M5b-2 (m5b2-plan.md D1) |

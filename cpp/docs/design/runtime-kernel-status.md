...
| Gate | Result |
|---|---|
| P1 kernel conformance | lifetime 131, sync 49, fields 22, collections 51, base 8 tests; Debug, ASan and Release green; protocol mutation tests fail when a protocol step is removed |
| P2 stress harness | 10-minute runs: 2x ASan Debug, 2x checked RelWithDebInfo; 30-minute checked RelWithDebInfo run of the final harness: PASS (backlog 0, census 0, zombie cuts 0, lockdep 0 cycles, no watchdog dumps, lag p99 156 ms); **30-minute ASan Debug run (2026-09-13): FAIL (liveness)** — no memory errors, canary failures 0, census 0, backlog 0 after teardown, but the Reclaimer collapsed: 268 scans in 30 min, lag p50 33 s / p99 73 s, backlog up to 7.0M objects / 1.4 GB, private memory 15.9 GB. Cause: every scan re-walks and copies the whole retired queue (O(backlog)) and the epoch advances once per scan, so a publisher pinned by a long blocking wait (10 s) grows the backlog until scans take seconds (positive feedback, amplified by ASan). Fix in progress: epoch-bucketed retire queue (scan only eligible buckets), epoch advance independent of scan work |

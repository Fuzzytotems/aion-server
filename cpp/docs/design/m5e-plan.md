...
| W-09 | a Daeva releases a charge skill (6 by 20) — **and a starting class too**: on 2026-09-25 a level-9 Muse (ARTIST) released a charge skill in a real-client session and the server logged "sent CM_USE_CHARGE_SKILL, which is not ported yet" (m5c0-client-session.md F-4), so this reaches the start maps before any class change | `CM_USE_CHARGE_SKILL` — no file: the charge never fires | W | C-04 |

...
| Run model | One GS per run. Cases run in fixed order. Case 7 writes the stop file with account B online. |
...
| V9 | `SM_STATS_INFO` and `SM_PLAYER_INFO`: base max HP/MP equal the oracle's PlayerStatCalculator values; current max ≥ base. The passive-skill delta of `AION_PARTIAL` O-09 is on the allow-list. |
...
| V2 | For non-walker, non-pool NPCs: x/y/z equal a spot of that id (±0.01); heading and template level equal; HP% 100; the body decodes exactly. |
...
| V4 | SM_GATHERABLE_INFO ids ⊆ the gather spots within 100 m. |
...
| Q5 | New LS and GS login without a kick. SM_CHARACTER_LIST: 1 entry, level 1, map 210010000, position T, appearance bytes equal those of creation, visible equipment = the equipped items. Enter gives SM_PLAYER_SPAWN at T. CM_QUIT(0). |
...
| Q8 | Reports (the LS is stopped with CTRL_BREAK and has exited): `unported_trace.txt` empty; `partial_trace.txt` ⊆ the allow-list; `census.txt` empty; `live_counts.txt` has 0 Player/Item/PlayerCommonData/Account/PlayerStorage; no "did not leave world cleanly", zombie-cut or stale-pin line; lockdep empty; no watchdog dump; no ERROR line in either log; `m5a_summary.txt` lists no "not ported yet" CM from the scripted path. |
...
| 7-10 | `SM_QUEST_COMPLETED_LIST+`, `SM_QUEST_LIST`, `SM_TITLE_INFO`, `SM_MOTION` |
...
- `gs.smoke.startup` (updated by F-02) and `gs.m4.check_static_data` keep passing.

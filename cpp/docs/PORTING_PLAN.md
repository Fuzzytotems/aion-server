...
| 4 | game-server foundation: static data (JAXB replacement), geo, world, object model, DAOs, network packets | All static data loads and counts match the Java server | ✅ 2026-09-15: **M4 passed** (tag `milestone-m4`): `aion_game_server --check-static-data` loads config, database, IDFactory, all static data, geo and the world with counts equal to the independent oracles (CTest `gs.m4.check_static_data`; RelWithDebInfo startup 3.9 s, 1.5 GB). Steps: design; runtime kernel P1-P4 ([status](design/runtime-kernel-status.md)); wave 1 tooling and leaf libraries ([status](design/wave1-status.md)); spine S0a/S0b/S0c frozen as tag `spine-v1` ([status](design/spine-status.md)); porting waves ([status](design/phase4-status.md)): 3a-1 `a99ec5fcb` "wave 3a-1: base, templates, objects, player, network", 3a-2 `fee633404` "wave 3a-2: geo, templates, skill/quest shells, items", 3b-1 `58b6527d7` "wave 3b-1 and milestone M4: all static data loads", 3b-2 (server packets SM_A..SM_Z, runtime hardening and M4 performance; its final tree still needs the full verification before the commit). What phase 5 inherits: phase4-status.md "Phase 4 close-out" |

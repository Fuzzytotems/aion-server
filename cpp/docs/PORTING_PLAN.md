...
| 4 | game-server foundation: static data (JAXB replacement), geo, world, object model, DAOs, network packets | All static data loads and counts match the Java server | in progress: design done; runtime kernel (P1-P4) implemented, all gates passed ([status](design/runtime-kernel-status.md)); wave 1 (generators, lint, oracles, configs, geo math, crypt, XML runtime, handler registry) done ([status](design/wave1-status.md)); next: spine (S0a chunk manifest and skeleton, S0b hub headers, S0c generated declarations) |

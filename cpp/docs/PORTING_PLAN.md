...
| 4 | game-server foundation: static data (JAXB replacement), geo, world, object model, DAOs, network packets | All static data loads and counts match the Java server | in progress: design done; runtime kernel (P1-P4) implemented, all gates passed ([status](design/runtime-kernel-status.md)); wave 1 (generators, lint, oracles, configs, geo math, crypt, XML runtime, handler registry) done ([status](design/wave1-status.md)); spine S0a/S0b/S0c done and frozen as tag spine-v1 ([status](design/spine-status.md)); next: phase 4 porting waves 3a-1, 3a-2, 3b toward M4 |
